...
//            Print();
            ...
            if(mark[i][j] == 0)
            {
                ...
                // 보의 한쪽 끝 위에 있거나
                if(mark[i-1][j] == 1) IsPossible = true;
                if(mark[i+1][j] == 1) IsPossible = true;
                // 다른 기둥 위에 있거나
                if(mark[i][j-1] == 0) IsPossible = true;
            }
            // 보
            else if(mark[i][j] == 1)
            {
                // 왼쪽 or 오른쪽에 기둥
                if(mark[i][j-1] == 0 || mark[i+1][j-1] == 0) IsPossible = true;
                // 양쪽에 보
                if(mark[i-1][j] == 1 && mark[i+1][j] == 1) IsPossible = true;
            }
