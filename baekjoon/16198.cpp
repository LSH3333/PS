...
int main()
{
    ...
    do {
        ...
        // 하나의 순열에 대한 계산
        int energy = 0;
        // idx 배열에는 순열이 저장되어 있다
        // {1,2,3}, {1,3,2} ... 이런식으로 permutation 함수에 의해 순열이 만들어진다
        // idx[in]의 in값을 증가시키면서 {1,2,3} 순열의 경우 1번구슬->2번구슬->3번구슬 순으로 뽑는다
        // {1,3,2} 순열의 경우 1번구슬->3번구슬->2번구슬 순으로 뽑는다
        int in = 1;
